...
public:
	...
	virtual void DrawUnicodeChar(wchar_t wch, FontDrawType_t drawType = FONT_DRAW_DEFAULT ) = 0;//25
	...
	virtual VPANEL GetModalPanel() = 0;//60
	...
	virtual void GetTextSize(HFont font, const wchar_t *text, int &wide, int &tall) = 0;//74
	...
	virtual bool ShouldPaintChildPanel(VPANEL childPanel) = 0;//80
	...
	virtual void DrawTexturedSubRect( int x0, int y0, int x1, int y1, float texs0, float text0, float texs1, float text1 ) = 0;//100
